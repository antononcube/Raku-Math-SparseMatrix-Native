...
class CSRStruct is repr('CStruct') {
    ...
    sub svd(CSRStruct is rw, CSRStruct is rw, CSRStruct is rw, CSRStruct, int32, int32, num64 --> int32)
            is native($library) {*}
    ...
    method svd(UInt:D $k = min($!nrow, $!ncol), :$max-steps is copy = Whatever, Numeric:D :tol(:$tolerance) = 1e-8) {
        die 'The argument $k is expected to be between 0 and min(nrow, ncol).'
        unless $k <= min($!nrow, $!ncol);

        if $max-steps.isa(Whatever) { $max-steps = -1 }
        die 'The argument $max-steps is expected to be integer or Whatever.'
        unless $max-steps ~~ Int:D;

        my $u = CSRStruct.new();
        my $s = CSRStruct.new();
        my $v = CSRStruct.new();
        my $status = svd($u, $s, $v, self, $k, $max-steps, $tolerance.Num);
